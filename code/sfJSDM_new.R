####
# sfJSDM function override with new CV functionality
#     1) save the CV divergence from each fold so we have an idea of variance
#         (by default a list where each element is a fold, no option to change)
#     2) allow the user to input the CV splits (folds) for blocks
#         (default is current folds, to override input list to 'k.supply.folds')
...
sfJSDM_new <- function(formula, data, inits, priors, 
                       ...
                       k.supply.folds = NA, k.sum.sites = TRUE, ...){
  ...
  if (!NNGP) {
    ...
    if (!missing(k.fold) & do.k.fold) {
      ...
      if(is.list(k.supply.folds)){
        if(length(k.supply.folds) != k.fold){
          stop("*~*~*~* ERROR: length(k.supply.folds) != k.fold\n............ how many folds do you want? *~*~*~*")
        }
        sites.random <- 1:J
        sites.k.fold <- k.supply.folds
        message("*~*~*~* NOTE: No adjustment for different block sizes *~*~*~*")
        if(setequal(unlist(sites.k.fold), 1:J) == FALSE){
          stop("*~*~*~* ERROR: at least one site is missing,\n 
               ........ use setdiff(1:num.sites, as.numeric(unlist(k.supply.folds))) to find it *~*~*~*")
        }
      } else if(!is.na(k.supply.folds)){
        stop("*~*~*~* ERROR: Supplying folds requires a 'list' object *~*~*~*")
      } else {
      ...
      model.deviance <- foreach (i = 1:k.fold) %dopar% {
