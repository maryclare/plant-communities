...
num_omp_threads <- 20
verbose         <- TRUE
num_report      <- 1000 # reports after number of batches
...
# Model formula
jsdm_formula <- ~ scale(tmax) + scale(tmin) + scale(soil) + scale(ppt) + 
  scale(vpd) + scale(elv) 
...
yhat <- temp_df[,1]
temp <- lm(yhat ~ scale(tmax) + scale(tmin) + scale(soil) + scale(ppt) + 
             scale(vpd) + scale(elv) ,
           data=temp_df)
...
# run the model
#####
out <- lfJSDM(formula = jsdm_formula, 
              data = data_list, 
              inits = inits, 
              n.samples = num_batch * batch_length, 
              priors = priors, 
...
saveRDS(out, paste0("nhp_herbs_northeast_k100_", num_factors, "factors_nonSpatialModelRun_initial_", Sys.Date(), ".rds"))
...
saveRDS(out, paste0("nhp_herbs_northeast_k100_", num_factors, "factors_nonSpatialModelRun_10foldCV_run2_", Sys.Date(), ".rds"))
